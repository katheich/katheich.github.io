import React from 'react'

const Skills = () => {

  return (<section className="" id="skills">
    <div className="container">
      <div className="title">
        <span className="backslash">/</span>
        <span className="backslash">/ </span>
        Tech I&apos;ve worked with
      </div>

      <div className="columns is-mobile is-multiline">
        
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-python-plain"></i><div className="label">Py&shy;thon</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-javascript-plain"></i><div className="label">Java&shy;Script</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-react-original"></i><div className="label">Re&shy;act&shy;.js</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-django-plain"></i><div className="label">Djan&shy;go</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-docker-plain"></i><div className="label">Docker</div></div>

        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-postgresql-plain"></i><div className="label">Post&shy;gre&shy;SQL</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-mongodb-plain"></i><div className="label">Mongo&shy;DB</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg height="64" width="64" xmlns="http://www.w3.org/2000/svg"><g fill-rule="evenodd" transform="matrix(.826767 0 0 .826767 8.416322 51.138164)"><path d="m39.1 15.557v-24.43l-21.16-12.22v24.43zm0-27.1-21.16-12.22v-24.44l21.16 12.22z" fill="#5c4ee5"/><path d="m62.6-23.773v-24.44l-21.17 12.22v24.44z" fill="#4040b2"/><path d="m15.62-25.193v-24.44l-21.16-12.22v24.44z" fill="#5c4ee5"/></g></svg>
          </figure>
          <div className="label">Terra&shy;form</div>
        </div>

        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="22.34 10.9 64 64" width="64" height="64"><path d="M37.17 42.916l-5.957-20.484-3.377 1.688v37.527l3.377 1.657z" fill="#9d5125"/><path d="M31.212 63.304l6.53-1.53V23.993l-6.53-1.53z" fill="#f58534"/><path d="M46.408 42.916l-6.18 24.912-4.97-2.485V20.425l4.97-2.485z" fill="#9d5125"/><path d="M40.228 17.94l10.13 3.25v43.325l-10.13 3.218z" fill="#f58534"/><path d="M77.468 57.76l-4.078-.223V26.7h4.078z" fill="#9d5125"/><path d="M77.15 26.7l.446 3.982-4.1.573-15.833-4.1L62.2 24.8z" fill="#6b3b19"/><path d="M77.723 55.085l-4.1-.573-15.833 4.1 4.556 2.357z" fill="#fcbf92"/><g fill="#9d5125"><path d="M67.083 32.658l-8.155 1.083v18.286l8.155 1.083z"/><path d="M61.126 42.852L54.34 10.9l-7.932 3.95v56.068l7.932 3.95z"/></g><path d="M77.468 55.15l-15.196 4.556v-33.64l15.196 4.556zM54.34 10.9v64l26.505-13.252V24.152zm12.743 42.274l8.442-10.258-8.442-10.258z" fill="#f58534"/></svg>
          </figure>
          <div className="label">AWS Lamb&shy;da</div>
        </div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-github-plain"></i><div className="label">Git&shy;Hub</div></div>

        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-nodejs-plain"></i><div className="label">Node&shy;.js</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-express-original"></i><div className="label">Ex&shy;press&shy;.js</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-webpack-plain"></i><div className="label">Web&shy;pack</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="fab fa-yarn"></i><div className="label">Yarn</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 0 24 24" height="50" xmlns="http://www.w3.org/2000/svg"><path d="M14.051 2.751l4.935 2.85c.816-.859 2.173-.893 3.032-.077.148.14.274.301.377.477.589 1.028.232 2.339-.796 2.928-.174.1-.361.175-.558.223v5.699c1.146.273 1.854 1.423 1.58 2.569-.048.204-.127.4-.232.581-.592 1.023-1.901 1.374-2.927.782-.196-.113-.375-.259-.526-.429l-4.905 2.832c.372 1.124-.238 2.335-1.361 2.706-.217.071-.442.108-.67.108-1.181.001-2.139-.955-2.14-2.136 0-.205.029-.41.088-.609l-4.936-2.847c-.816.854-2.171.887-3.026.07-.854-.816-.886-2.171-.07-3.026.283-.297.646-.506 1.044-.603l.001-5.699c-1.15-.276-1.858-1.433-1.581-2.584.047-.198.123-.389.224-.566.592-1.024 1.902-1.374 2.927-.782.177.101.339.228.48.377l4.938-2.85C9.613 1.612 10.26.423 11.39.088 11.587.029 11.794 0 12 0c1.181-.001 2.139.954 2.14 2.134.001.209-.03.418-.089.617zm-.515.877c-.019.021-.037.039-.058.058l6.461 11.19c.026-.009.056-.016.082-.023V9.146c-1.145-.283-1.842-1.442-1.558-2.588.006-.024.012-.049.019-.072l-4.946-2.858zm-3.015.059l-.06-.06-4.946 2.852c.327 1.135-.327 2.318-1.461 2.645-.026.008-.051.014-.076.021v5.708l.084.023 6.461-11.19-.002.001zm2.076.507c-.39.112-.803.112-1.192 0l-6.46 11.189c.294.283.502.645.6 1.041h12.911c.097-.398.307-.761.603-1.044L12.597 4.194zm.986 16.227l4.913-2.838c-.015-.047-.027-.094-.038-.142H5.542l-.021.083 4.939 2.852c.388-.404.934-.653 1.54-.653.627 0 1.19.269 1.583.698z"/></svg>          
          </figure>
          <div className="label">Graph&shy;QL</div>
        </div>

        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-sass-original"></i><div className="label">SASS</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-materialui-plain"></i><div className="label">Material&shy;UI</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-heroku-original"></i><div className="label">He&shy;ro&shy;ku</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-visualstudio-plain"></i><div className="label">VS&shy;Code</div></div>
        <div className="column is-3-mobile is-one-fifth-desktop"><i className="devicon-jupyter-plain"></i><div className="label">Ju&shy;py&shy;ter</div></div>
        
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 0 129 129" height="50" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
              <g
                id="XMLID_1_"
                transform="translate(-67.522309,-187.56308)"><path
                  id="XMLID_10_"
                  className="st1"
                  d="m 76.7,253.3 c 0,30.4 24.6,55 55,55 30.4,0 55,-24.6 55,-55 0,-30.4 -24.6,-55 -55,-55 -30.4,0 -55,24.6 -55,55"
                  fill="#000000"
                />
                <path
                  id="XMLID_11_"
                  className="white"
                  d="m 108.5,210 c -4.2,0.2 -8.3,0.7 -12.4,1.3 -11.5,9.7 -18.9,24.1 -19.4,40.2 3.2,3.4 7.5,11.1 12.5,15.6 0.7,-0.3 1.4,-0.5 2.1,-0.8 -2.4,-2.8 -5.8,-6.5 -6.3,-8.3 -2.5,-8.4 0.4,-16.3 6.6,-16.9 5.6,-0.6 10.4,-8.9 14.3,-14.4 8.1,-11.2 20.6,-9.4 20,-15 -0.7,-3.3 -2.7,-4.3 -5.1,-4.3 -3.7,0.2 -8.6,2.4 -12.3,2.6"
                  fill="#ffffff"
                />
                <path
                  id="XMLID_12_"
                  className="white"
                  d="m 93.7,265.7 c -0.8,0.2 -1.6,0.4 -2.4,0.7 -0.7,0.2 -1.2,0.4 -2.1,0.8 -2,0.9 -3.5,2.3 -3.8,5.2 -0.6,5.3 4.5,9.5 8.7,10.8 5.4,1.6 5.9,-0.3 8.7,3.6 2,2.7 2.4,7.7 3.4,11 0.6,1.8 1.3,3.7 2.3,5.5 3.9,1.8 8,3.2 12.3,4 1.1,-6.3 0.6,-14.9 2,-18.6 2.9,-7.6 4.5,-10.9 0.6,-18.7 -8.6,-2.4 -11.3,-3.2 -20,-5 -1.8,-0.4 -2.9,-0.5 -3.9,-0.5 -1.4,-0.1 -2.7,0.3 -5.8,1.2"
                  fill="#ffffff"
                />
                <path
                  id="XMLID_13_"
                  className="white"
                  d="m 147.2,221.9 c -2.5,3.6 -11.5,4.6 -14.4,8.7 -2.2,3.2 0.1,9.5 -1.9,13.1 -0.9,1.7 -0.5,6.5 0.6,8.1 3.4,5 9.2,2.2 13.5,4 5.2,2.2 5.9,8.3 10.2,12.7 4.4,4.5 -5.6,11.3 2.5,27.6 0.9,1.7 1.9,3 3,3.9 5.5,-3.4 10.3,-7.8 14.3,-12.8 1.2,-8.2 -1.5,-10.8 1.9,-18.3 2.1,-4.6 5.8,-12.6 9.7,-18.1 -0.7,-15.2 -7.5,-28.8 -18,-38.3 -1,0.3 -2.2,0.3 -4,0 -1.5,-0.3 -2.8,-0.4 -4,-0.4 -8.1,0 -10.5,5.6 -13.4,9.8"
                  fill="#ffffff"
                />
                <path
                  id="XMLID_33_"
                  className="st0"
                  d="m 125.4,311.7 c -1.8,-1 -4.1,-2.7 -6.2,-4.1 -0.9,0.8 -1.6,1.9 -2.3,2.9 2.4,0.8 5.7,0.8 8.5,1.2 m -9.6,-6.6 c -3.9,-2.9 -13.6,-12.1 -17.6,-17.6 -1.5,0.9 -3.1,3.2 -5.2,3.6 -0.8,0.2 -2.7,0.4 -3.3,0.2 -0.6,-0.2 0.1,-1.4 0.7,-2.2 1,-1.4 2.6,-3.1 4.8,-4.9 -4.8,-7 -7,-12.1 -10.3,-20.5 -1.9,1.2 -3.2,2.2 -4.9,3.5 -0.5,0.4 -2.1,0.7 -2.8,0 -1.3,-1.2 -1.4,-3.3 0.5,-4.6 0.8,-0.6 4.6,-3.2 5.5,-3.9 -2.3,-6.7 -4.3,-15.6 -4.8,-20.2 -1.6,1.4 -3.8,2.9 -4.4,3.3 -0.9,0.3 -1.5,0 -1.7,-0.1 -0.9,9.5 0.1,20.1 3,29.5 4.9,16.3 18.6,31.9 37,37.7 0.5,-0.5 2.6,-3 3.5,-3.8 M 77.9,233.6 c -0.2,-3.5 -0.5,-8.4 -0.5,-8.4 -2.6,5.6 -4.1,9 -4.9,15.6 1.9,-3.1 5.4,-7.2 5.4,-7.2 m 27.7,-40.3 c 10.7,-4.4 21.2,-6.9 33.2,-5.2 1.3,0.2 1.2,1.3 -0.2,1.3 h -1.7 c -9.7,0.2 -24,3.2 -31.4,7.4 -0.2,4.9 0.8,12.8 1.8,18.9 6.7,-2.8 17.1,-5.7 25,-6.6 -4.5,-4.8 -10.6,-9.3 -11.3,-9.8 -0.7,-0.5 -1.1,-1.4 -1.1,-2.6 0.1,-2.1 1.9,-2.4 2.9,-2.2 3.3,-0.6 6.5,-1.3 12.1,-1.2 23.5,-1 44.9,12.3 53.1,27.4 20.7,32.8 0,78 -27.1,90.1 -11.6,5.2 -21.6,7.4 -39.9,4.8 C 102.6,313 87.6,304 75.9,285.1 61.5,261.8 69.6,234.7 71.6,229 c 8.7,-20.2 22.9,-31 34,-35.7 m -24.3,24.8 c -0.3,2.5 0,7 0.6,12.4 5,-4.5 14.6,-10.3 22.7,-13.9 -0.9,-6.1 -1.9,-15.9 -1.8,-18.6 -7.5,4.1 -15.6,12.3 -21.6,20.8 m 1.2,16.6 c 0.7,4.9 3,15.4 5.1,20.7 5,-3.8 16.3,-10.7 22.7,-13.6 -2.2,-8.2 -3.7,-13.3 -5,-21.5 -8.9,4.1 -18.5,10.3 -22.8,14.4 m 7.1,25.4 c 3,8.5 5.8,14.9 9.6,20.2 4.4,-3.8 13.4,-9.8 20.2,-13.1 -2.3,-4.9 -5.5,-14 -7.8,-20.8 -8.8,4.5 -15.4,8.7 -22,13.7 m 12.4,24 c 4.7,6 11.9,13.5 17,17.7 4.8,-3.9 10.2,-7.5 14.8,-9.7 -3.9,-6.1 -9.7,-14.8 -12.1,-20.3 -8.7,3.9 -13.6,7.1 -19.7,12.3 m 20,20.3 c 1.8,1.9 9.2,6.7 9.2,6.7 4.9,0.2 11,-0.4 15.4,-1.1 -2.4,-2.8 -8.5,-11.3 -10.3,-14.2 -4.7,2 -10.6,5.6 -14.3,8.6 m 29.7,4.4 c 2.5,-0.8 8.8,-3.6 10.1,-4.5 0.3,-5.1 0.3,-10.8 0.1,-14.3 -6.6,0 -15.9,1.9 -21.2,4.2 3,5 7.2,10.4 11,14.6 m 13.6,-6.4 c 5,-3.1 9.6,-6.7 13,-10.9 -2.9,-0.7 -9.4,-1.6 -12.6,-1.4 -0.1,0 -0.2,7.7 -0.4,12.3 m 14.5,-12.9 c 0.9,-0.9 2.3,-2.9 2.9,-4.1 0.5,-4.7 0.6,-14.3 0.4,-17.1 -5.7,-2.3 -14.4,-4.1 -20.8,-4.6 2.1,7.6 3.1,18.9 3.3,22.5 3,0.3 10.7,1.9 14.2,3.3 m 8.3,-21.8 c -0.3,0.9 -1.8,1.8 -2.6,1.6 0.1,1.6 -0.3,9.4 -0.7,12.5 1.1,-2.2 3.2,-7.8 3.8,-10.1 l 0.9,-3.5 c 0.9,-2.9 3.7,-13.1 1.1,-26.4 -2.9,-14.8 -8.2,-20.1 -10.1,-22.4 -4.8,-2.6 -12,-4.9 -12,-4.9 6.5,8.8 8.8,13 12.7,23.5 2.3,1.2 6.4,3.5 6.9,4.1 0.4,0.5 0.4,1.4 -0.9,1.4 -1.1,0 -3.2,-1 -4.8,-1.5 1.7,5.7 3,16.4 2.9,23.6 0.3,-0.2 3.1,1.2 2.8,2.1 M 177.1,215 c -9.1,-11.1 -29.5,-20.5 -52.3,-19.1 5.1,3.4 9.3,8.4 13.1,12.6 5.7,-0.5 17.7,0.4 22.2,1.7 -0.8,-0.9 -1.7,-1.8 -1.9,-2.3 -0.6,-1.4 0.9,-1.7 2.1,-0.9 1.6,1 3.4,2.2 4.8,3.6 2.5,0.6 9,2.8 12,4.4 m 5.8,50 c -0.4,-7.2 -1.5,-16.7 -4.1,-23.3 -7,-2.8 -16.6,-4.6 -25,-4.7 2.6,6.1 5.6,16.2 7.1,22.7 6,0.7 15.5,2.5 22,5.3 m -19.7,-51.1 c -6.3,-1.1 -15.6,-1.9 -22.5,-1.5 4.7,6.5 8.1,12.1 11.3,20.2 8.7,0.2 20.5,2.8 25.3,4.7 -2.8,-7.7 -8.3,-17.5 -14.1,-23.4 m -39.8,52.2 c 10.3,-4.8 21.7,-6.9 33.2,-6.6 -3.1,-10.8 -4.9,-15.8 -7.6,-22.4 -12.7,0.6 -23.6,3.7 -33.4,8.4 -0.1,-0.1 3.6,11.6 7.8,20.6 m 34.3,-2.1 c -13,-0.2 -24.3,2.9 -32,6.7 3.3,7.1 9.7,16.4 12.2,20.2 4.2,-1.6 8.7,-2.7 12.2,-3.3 3.5,-0.5 8.2,-1.1 11.4,-1 -0.6,-8.1 -1.5,-13.6 -3.8,-22.6 m -43.6,-23 c 9.4,-4 19.5,-7.2 33.2,-8.3 -3.5,-8.4 -7.3,-14.3 -11.9,-19.7 -10.1,0.8 -19.3,3.5 -27.3,6.8 1.2,6.2 3.4,13.7 6,21.2"
                  fill="#000000"
                />  
              </g>
            </svg>
          </figure>
          <div className="label">Arc&shy;GIS Pro</div>
        </div>

        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 -50 780 270" height="21" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
              <g
                transform="translate(-7.8319998,-800.29999)"
                id="layer1">
                <path
                  d="m 207.9,843.1 -14.6,38.5 H 96.44 c -20.53,0 -20.76,27 0,27 h 56.66 c 72.2,-0.1 72.2,102.4 0,102.4 H 42.3 l 15.22,-37.6 h 97.08 c 20.9,0 21.4,-27.3 0,-27.3 H 98.61 c -73.16,0 -73.39,-103 0,-103 z"
                  id="path3782"
                  fill="#ffffff" stroke="none"
                />
                <path
                  d="m 212,843.1 -14.6,38.5 h 46.1 V 1011 h 37.9 V 881.6 h 36.4 l 14.6,-38.5 z"
                  id="path3792"
                  fill="#ffffff" stroke="none"
                />
                <path
                  d="m 335.4,843.1 -14.6,38.5 h 108.5 c 7.3,0 13.5,4.4 13.5,12.6 v 14.4 h -85.9 c -66.3,0 -66.6,102.4 0,102.4 H 480.6 V 893.6 c 0,-24.3 -20.4,-50.5 -53.3,-50.5 z m 23.5,103 h 83.9 v 27.3 h -83.9 c -19.6,0 -19,-27.3 0,-27.3 z"
                  id="path3794"
                  fill="#ffffff" stroke="none"
                />
                <path
                  d="m 619.7,843.1 -14.6,38.5 h 108.5 c 7.4,0 13.6,4.4 13.6,12.6 v 14.4 h -85.9 c -66.3,0 -66.6,102.4 0,102.4 H 764.9 V 893.6 c 0,-24.3 -20.4,-50.5 -53.2,-50.5 z m 23.5,103 h 84 v 27.3 h -84 c -19.5,0 -19,-27.3 0,-27.3 z"
                  id="path3794-5"
                  fill="#ffffff" stroke="none"
                />
                <path
                  d="M 495.6,843.1 481,881.6 h 46.1 V 1011 h 37.8 V 881.6 h 36.4 L 616,843.1 Z"
                  id="path3792-3"
                  fill="#ffffff" stroke="none"
                />
              </g>
            </svg>
          </figure>
          <div className="label">Sta&shy;ta</div>
        </div>
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 0 24 24" height="50" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg"><path d="M4.323 16.248C3.13 15.354 1.64 14.31 0 13.118l5.814-2.236 2.385 1.789c-1.789 2.087-2.981 2.832-3.876 3.578zm15.95-6.26c-.447-1.193-.745-2.385-1.193-3.578-.447-1.342-.894-2.534-1.64-3.578-.298-.447-.894-1.491-1.64-1.491-.149 0-.298.149-.447.149-.447.149-1.043 1.043-1.193 1.64-.447.745-1.342 1.938-1.938 2.683-.149.298-.447.596-.596.745-.447.298-.894.745-1.491 1.043-.149 0-.298.149-.447.149-.447 0-.745.298-1.043.447-.447.447-.894 1.043-1.342 1.491 0 .149-.149.298-.298.447l2.236 1.64c1.64-1.938 3.578-3.876 4.919-7.602 0 0-.447 4.025-4.025 8.348-2.236 2.534-4.025 3.876-4.323 4.174 0 0 .596-.149 1.193.149 1.193.447 1.789 2.087 2.236 3.279.298.894.745 1.64 1.043 2.534 1.193-.298 1.938-.745 2.683-1.491s1.491-1.64 2.236-2.385c1.342-1.64 2.981-3.727 5.068-2.683.298.149.745.447.894.596.447.298.745.596 1.193 1.043.745.596 1.043 1.043 1.64 1.342-1.491-2.981-2.534-5.963-3.727-9.093z"/></svg>
          </figure>
          <div className="label">Mat&shy;lab</div>
        </div>
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 0 24 24" height="45" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg"><path d="M0 .3v23.4h24V.3H0zm1 1h22v21.4H1V1.3zm7.775 15.218c0 .1-.017.132-.132.132H7.107c-.1 0-.132-.05-.132-.132V5.782c0-.1.05-.132.132-.132H8.66c.082 0 .115.033.115.116v10.752zm1.77-3.872c0-2.393 1.578-4.257 4.102-4.257.214 0 .33 0 .528.015V5.75c0-.066.05-.1.1-.1H16.9c.084 0 .1.034.1.084v9.355c0 .28 0 .625.05 1.004 0 .066-.02.083-.087.116-.858.413-1.766.595-2.624.595-2.213 0-3.797-1.37-3.797-4.158zm4.63-2.624c-.148-.066-.347-.1-.594-.1-1.285 0-2.187.99-2.187 2.64 0 1.882.918 2.64 2.073 2.64.247 0 .512-.032.71-.115v-5.065z"/></svg>
          </figure>
          <div className="label">In&shy;Design</div>
        </div>
        <div className="column is-3-mobile is-one-fifth-desktop">
          <figure className="image is-square is-paddingless">
            <svg viewBox="0 0 24 24" height="45" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg"><path d="M0 .3v23.4h24V.3H0zm1 1h22v21.4H1V1.3zm4.8 4.48c0-.067.14-.116.224-.116.644-.033 1.588-.05 2.578-.05 2.772 0 3.85 1.52 3.85 3.466 0 2.54-1.842 3.63-4.102 3.63-.38 0-.51-.017-.775-.017v3.842c0 .083-.033.116-.115.116H5.916c-.083 0-.115-.03-.115-.113V5.78zm1.775 5.312c.23.016.412.016.81.016 1.17 0 2.27-.412 2.27-1.996 0-1.27-.786-1.914-2.122-1.914-.396 0-.775.016-.957.033v3.864zm8.607-1.188c-.792 0-1.056.396-1.056.726 0 .363.18.61 1.237 1.155 1.568.76 2.062 1.485 2.062 2.557 0 1.6-1.22 2.46-2.87 2.46-.876 0-1.62-.183-2.05-.43-.065-.033-.08-.082-.08-.165V14.74c0-.1.048-.133.114-.084.624.413 1.352.594 2.012.594.792 0 1.122-.33 1.122-.776 0-.363-.23-.677-1.237-1.205-1.42-.68-2.014-1.37-2.014-2.527 0-1.287 1.006-2.36 2.755-2.36.86 0 1.464.132 1.794.28.082.05.1.132.1.198v1.37c0 .083-.05.133-.15.1-.444-.264-1.1-.43-1.743-.43z"/></svg>
          </figure>
          <div className="label">Photo&shy;shop</div>
        </div>


        

      </div>



    </div>
  </section>)

}

export default Skills

